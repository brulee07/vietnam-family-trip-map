...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='13.2';
...
let local={routes:{},notes:{},notePositions:{},memoPhotos:{},saved:[],custom:[],aiUndo:{},todayProgress:{},checklists:{},meta:{}};
...
if(!local.notePositions||typeof local.notePositions!=='object'||Array.isArray(local.notePositions))local.notePositions={};
if(!local.memoPhotos||typeof local.memoPhotos!=='object'||Array.isArray(local.memoPhotos))local.memoPhotos={};
...
// Retired built-in places only; never discard unrelated user-created places.
const RETIRED_PLACES=new Set(['quy_nhon:gold','tuy_hoa:giatuituyhoa','tuy_hoa:giatsayphuyen','nha_trang:laundry2h','nha_trang:washgo']);
function cleanRetiredPlaces(state){
  const keep=k=>!RETIRED_PLACES.has(k);
  for(const [rk,rows] of Object.entries(state.routes||{})){
    if(!Array.isArray(rows))continue;
    const gap=n=>Number.isInteger(n)?n-rows.slice(0,n).filter(k=>!keep(k)).length:n;
    if(rows.some(k=>!keep(k))){
      for(const m of Object.values(state.memoCards||{}))if(m.routeKey===rk)m.position=gap(m.position);
      if(state.notePositions&&rk in state.notePositions)state.notePositions[rk]=gap(state.notePositions[rk]);
      state.routes[rk]=rows.filter(keep);
    }
  }
  if(Array.isArray(state.saved))state.saved=state.saved.filter(keep);
  if(Array.isArray(state.custom))state.custom=state.custom.filter(p=>keep(key(p)));
  for(const p of Object.values(state.todayProgress||{}))if(Array.isArray(p?.done))p.done=p.done.filter(keep);
  for(const u of Object.values(state.aiUndo||{}))for(const k of ['before','after'])if(Array.isArray(u?.[k]))u[k]=u[k].filter(keep);
  return state;
}
cleanRetiredPlaces(local);
function save(){cleanRetiredPlaces(local);MemoCards.migrate(local);try{localStorage.setItem(KEY,JSON.stringify(local));return true;}catch{toast('이 기기에 저장할 수 없습니다. 여행 도구에서 파일로 백업해 주세요.');return false;}}
...
const memoMigrationPending=MemoCards.migrate(local);
if(memoMigrationPending){try{localStorage.setItem(KEY,JSON.stringify(local));}catch{/* Retry through save. */}}
function routeMemos(rk=routeKey()){return MemoCards.list(local,rk);}
function memoGap(m,count){return m.position===null?count:Math.max(0,Math.min(count,m.position));}
function memosAt(gap,count,rk=routeKey()){return routeMemos(rk).filter(m=>memoGap(m,count)===gap);}
function patchMemo(id,changes){return MemoCards.patch(local,id,changes);}
function deleteMemo(id){const m=local.memoCards[id];if(!m||m.deleted)return;patchMemo(id,{deleted:true});save();closeSheet();render();toast('메모를 삭제했습니다.');}
function memoPhotoSrc(photo){return photo?(photo.dataUrl||photo.remoteUrl||photo.url||''):'';}
function hasRouteMemo(rk=routeKey()){return routeMemos(rk).length>0;}
function memoPhotoImg(photo,cls='memo-photo-thumb'){const src=memoPhotoSrc(photo);return src?`<img class="${cls}" src="${esc(src)}" alt="메모 첨부 사진" loading="lazy">`:'';}
function readBlobAsDataURL(blob){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||''));r.onerror=reject;r.readAsDataURL(blob);});}
async function loadImageFile(file){const url=URL.createObjectURL(file);try{return await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(Error('image'));img.src=url;});}finally{setTimeout(()=>URL.revokeObjectURL(url),1000);}}
async function compressMemoPhoto(file){
  if(!file||!String(file.type||'').startsWith('image/'))throw Error('type');
  if(file.size>20*1024*1024)throw Error('size');
  const img=await loadImageFile(file),maxSide=960,scale=Math.min(1,maxSide/Math.max(img.naturalWidth||1,img.naturalHeight||1));
  const w=Math.max(1,Math.round(img.naturalWidth*scale)),h=Math.max(1,Math.round(img.naturalHeight*scale)),canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;
  const ctx=canvas.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);ctx.drawImage(img,0,0,w,h);
  let q=.8,blob=null;for(let i=0;i<5;i++){blob=await new Promise(r=>canvas.toBlob(r,'image/jpeg',q));if(!blob)throw Error('encode');if(blob.size<=160*1024||q<=.48)break;q-=.08;}
  const dataUrl=await readBlobAsDataURL(blob);return {dataUrl,width:w,height:h,size:blob.size,name:String(file.name||'memo-photo.jpg').slice(0,120),updatedAt:new Date().toISOString()};
}
function openMemoPhoto(photo){if(!photo)return;const src=memoPhotoSrc(photo);if(!src)return;const shared=!!(photo.remoteId&&(photo.remoteUrl||photo.url));openSheet('메모 사진',`<img class="memo-photo-full" src="${esc(src)}" alt="메모 첨부 사진 크게 보기"><p class="notice">${shared?'가족 공유에 동기화된 메모 사진입니다.':'현재 기기에 저장된 사진입니다. 온라인 연결 시 가족 사진 동기화를 시도합니다.'}</p>`,`<button class="btn primary" id="memo-photo-done">확인</button>`);$('#memo-photo-done').onclick=closeSheet;}
function notePlacementOptions(rows,selected=null){const opts=[`<option value="bottom" ${selected===null?'selected':''}>일정 아래에 표시</option>`,`<option value="0" ${selected===0?'selected':''}>첫 장소 앞</option>`];for(let i=1;i<=rows.length;i++){const a=place(rows[i-1]),b=i<rows.length?place(rows[i]):null,label=b?`${i}번과 ${i+1}번 사이 · ${a?.name||''} → ${b?.name||''}`:`${i}번 뒤 · ${a?.name||''} 다음`;opts.push(`<option value="${i}" ${selected===i?'selected':''}>${esc(label)}</option>`);}return opts.join('');}
function dragHandle(label){return edit?`<button class="route-drag-handle" data-route-drag-handle aria-label="${esc(label)} 드래그해서 순서 변경" title="드래그해서 순서 변경"><span aria-hidden="true">⠿</span></button>`:'';}
function routeMemoBlock(m,pos,rows){
  const photo=m.photo,id=esc(m.id);
  return `<div class="route-memo-stop route-sort-item" data-sort-kind="memo" data-sort-key="${id}"><span class="route-memo-dot">✎</span><div class="route-drag-shell"><div class="route-memo-card"><button class="route-memo-main" data-memo-edit="${id}"><span><small>메모${photo?' · 사진':''}</small><b>${esc(m.text||'사진 메모')}</b>${photo&&!memoPhotoSrc(photo)?'<small>사진 업로드 대기</small>':''}</span></button>${memoPhotoSrc(photo)?`<button class="memo-photo-thumb-button" data-memo-photo="${id}" aria-label="메모 사진 크게 보기">${memoPhotoImg(photo)}</button>`:''}</div>${dragHandle('메모')}</div>${edit?`<div class="edit-row"><button data-memo-move="${id}" data-delta="-1">↑ 위로</button><button data-memo-move="${id}" data-delta="1">↓ 아래로</button><button class="danger" data-memo-delete="${id}">메모 삭제</button></div>`:''}</div>`;
}
function compactMemo(m,cls='today-route-memo'){
  return `<button class="${cls}" data-memo-edit="${esc(m.id)}"><span>✎</span><b>${esc(m.text||'사진 메모')}</b>${memoPhotoSrc(m.photo)?memoPhotoImg(m.photo,'today-memo-thumb'):m.photo?'<small>사진 업로드 대기</small>':''}</button>`;
}
function bindMemos(){
  document.querySelectorAll('[data-memo-edit]').forEach(b=>b.onclick=()=>editMemo(b.dataset.memoEdit));
  document.querySelectorAll('[data-memo-photo]').forEach(b=>b.onclick=e=>{e.stopPropagation();openMemoPhoto(local.memoCards[b.dataset.memoPhoto]?.photo);});
  document.querySelectorAll('[data-memo-delete]').forEach(b=>b.onclick=()=>{if(confirm('이 메모를 삭제할까요?'))deleteMemo(b.dataset.memoDelete);});
  document.querySelectorAll('[data-memo-move]').forEach(b=>b.onclick=()=>moveMemo(b.dataset.memoMove,Number(b.dataset.delta)));
}
function scheduleTimeline(rows){let html='';for(let gap=0;gap<=rows.length;gap++){html+=memosAt(gap,rows.length).map(m=>routeMemoBlock(m,gap,rows)).join('');
if(gap<rows.length){const k=rows[gap],p=place(k);if(p)html+=`<div class="stop route-sort-item" data-sort-kind="place" data-sort-key="${esc(k)}"><span class="dot">${gap+1}</span><div class="route-drag-shell"><button class="card card-media" data-place="${esc(k)}">${cardPhoto(p)}<span class="card-copy"><b>${esc(p.name)}</b><small>${esc(p.cat)}${p.stay?' · '+esc(p.stay):''}</small></span></button>${dragHandle(p.name)}</div>${edit?`<div class="edit-row"><button data-move="${gap}" data-delta="-1" ${gap===0?'disabled':''} aria-label="${esc(p.name)} 위로">↑ 위로</button><button data-move="${gap}" data-delta="1" ${gap===rows.length-1?'disabled':''}>↓ 아래로</button><button class="danger" data-remove="${gap}">일정에서 빼기</button></div>`:''}</div>`;}}return html;}
function saveMemoSequence(seq){
  let gap=0,order=0;
  for(const x of seq){if(x.kind==='place'){gap++;order=0;}else{const m=local.memoCards[x.key];if(m&&!m.deleted&&(m.position!==gap||m.order!==order))patchMemo(m.id,{position:gap,order});order++;}}
}
function commitRouteDrag(container,draggedKind){
  const seq=[...container.querySelectorAll(':scope > [data-sort-kind]')].map(el=>({kind:el.dataset.sortKind,key:el.dataset.sortKey||''}));
  local.routes[routeKey()]=seq.filter(x=>x.kind==='place').map(x=>x.key);
  saveMemoSequence(seq);save();render();toast(draggedKind==='memo'?'메모 위치를 변경했습니다.':'경로 순서를 변경했습니다.');
}
function moveMemo(id,delta){
  const rows=route(),seq=[];
  for(let gap=0;gap<=rows.length;gap++){memosAt(gap,rows.length).forEach(m=>seq.push({kind:'memo',key:m.id}));if(gap<rows.length)seq.push({kind:'place',key:rows[gap]});}
  const i=seq.findIndex(x=>x.kind==='memo'&&x.key===id),j=i+delta;if(i<0||j<0||j>=seq.length)return;
  [seq[i],seq[j]]=[seq[j],seq[i]];saveMemoSequence(seq);save();render();
}
function bindRouteDrag(){
  const container=$('#schedule-timeline');if(!edit||!container)return;
  container.querySelectorAll('[data-route-drag-handle]').forEach(handle=>handle.addEventListener('pointerdown',e=>{
    if(e.pointerType==='mouse'&&e.button!==0)return;
    const item=handle.closest('[data-sort-kind]');if(!item)return;
    e.preventDefault();e.stopPropagation();
    const draggedKind=item.dataset.sortKind,rect=item.getBoundingClientRect(),offsetY=e.clientY-rect.top;
    const ph=document.createElement('div');ph.className='route-drag-placeholder';ph.style.height=`${Math.max(58,rect.height)}px`;
    item.after(ph);item.classList.add('route-drag-source');item.style.display='none';
    const ghost=item.cloneNode(true);ghost.classList.add('route-drag-ghost');ghost.classList.remove('route-drag-source');ghost.style.display='block';ghost.style.left=`${rect.left}px`;ghost.style.top=`${rect.top}px`;ghost.style.width=`${rect.width}px`;ghost.querySelectorAll('button').forEach(b=>b.tabIndex=-1);document.body.appendChild(ghost);
    document.body.classList.add('route-dragging');
    try{handle.setPointerCapture(e.pointerId);}catch{}
    const move=ev=>{
      ev.preventDefault();ghost.style.top=`${Math.max(8,Math.min(innerHeight-58,ev.clientY-offsetY))}px`;
      const others=[...container.querySelectorAll(':scope > [data-sort-kind]')].filter(x=>x!==item&&x.style.display!=='none');let placed=false;
      for(const target of others){const r=target.getBoundingClientRect();if(ev.clientY<r.top+r.height/2){container.insertBefore(ph,target);placed=true;break;}}
      if(!placed)container.appendChild(ph);
      const edge=84;if(ev.clientY<edge)scrollBy(0,-14);else if(ev.clientY>innerHeight-edge)scrollBy(0,14);
    };
    const finish=ev=>{
      ev?.preventDefault?.();window.removeEventListener('pointermove',move,{capture:true});window.removeEventListener('pointerup',finish,{capture:true});window.removeEventListener('pointercancel',cancel,{capture:true});
      item.style.display='';container.insertBefore(item,ph);ph.remove();ghost.remove();item.classList.remove('route-drag-source');document.body.classList.remove('route-dragging');
      commitRouteDrag(container,draggedKind);
    };
    const cancel=()=>{
      window.removeEventListener('pointermove',move,{capture:true});window.removeEventListener('pointerup',finish,{capture:true});window.removeEventListener('pointercancel',cancel,{capture:true});
      item.style.display='';ph.remove();ghost.remove();item.classList.remove('route-drag-source');document.body.classList.remove('route-dragging');render();
    };
    window.addEventListener('pointermove',move,{capture:true,passive:false});window.addEventListener('pointerup',finish,{capture:true,passive:false});window.addEventListener('pointercancel',cancel,{capture:true,passive:false});
  },{passive:false}));
}
...
function todayView(){
  ...
  ${nextBlock}
  <section class="today-section"><div class="today-section-head"><div><span class="eyebrow">한눈에 보기</span><h2>오늘 일정</h2></div><div class="today-section-actions"><button class="textbtn" id="today-map">지도</button><button class="textbtn" id="today-schedule">일정 편집</button></div></div><div class="today-route-list">${(()=>{let out='';for(let gap=0;gap<=rows.length;gap++){out+=memosAt(gap,rows.length).map(m=>compactMemo(m)).join('');if(gap<rows.length){const x=rows[gap];out+=`<div class="today-stop ${done.has(x.k)?'done':''} ${nextRow?.k===x.k?'next':''}"><button class="today-stop-main" data-place="${esc(x.k)}"><span class="today-num">${done.has(x.k)?'✓':gap+1}</span><span><b>${esc(x.p.name)}</b><small>${esc(x.p.cat)}${x.p.stay?' · '+esc(x.p.stay):''}</small></span></button><button class="today-check" data-today-toggle="${esc(x.k)}" aria-label="${done.has(x.k)?'완료 취소':'완료 표시'}">${done.has(x.k)?'완료':'체크'}</button></div>`;}}return out||'<div class="empty">오늘 일정이 비어 있습니다.</div>';})()}</div></section>
  ...
  <button class="btn" id="today-memo">＋ 메모 추가</button>`;
  ...
  $('#today-map').onclick=()=>go('map',true);$('#today-schedule').onclick=()=>go('schedule');if($('#today-memo'))$('#today-memo').onclick=()=>editMemo();bindMemos();bindPlaces();
...
function editMemo(id=null){
  const rows=route(),rk=routeKey(),existing=typeof id==='string'?local.memoCards[id]:null;if(existing?.deleted)return;id=existing?.id||crypto.randomUUID();const existingPhoto=existing?.photo||null;let pendingPhoto=existingPhoto,photoRemoved=false,photoBusy=false;
  const has=!!existing,photoIcon=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5h10.5v2H6v9h12v-5h2v7H4z"/><path d="M7 14l2.5-3 2.2 2.4 1.8-2 3.5 4.1H7z"/><path d="M18 2v3h3v2h-3v3h-2V7h-3V5h3V2z"/></svg>`;
  openSheet(has?'메모 편집':'메모 추가',`<div class="memo-editor-top"><span>${dateLabel(date)}</span><button class="memo-photo-add" id="memo-photo-add" aria-label="사진 추가">${photoIcon}</button><input type="file" id="memo-photo-input" accept="image/*" hidden></div><label class="field memo-text-field"><textarea id="day-memo" maxlength="2000" placeholder="잊기 쉬운 정보를 메모해보세요.">${esc(existing?.text||'')}</textarea></label><div id="memo-photo-preview" class="memo-photo-preview"></div><label class="field"><span>메모 표시 위치</span><select id="memo-position">${notePlacementOptions(rows,existing?.position??null)}</select></label><p class="notice">가족 공유방에 연결되어 있으면 사진도 가족 휴대폰에 동기화됩니다. 오프라인에서는 이 기기에 먼저 저장하고, 다시 온라인이 되면 업로드를 시도합니다. 저장 후 일정 편집 모드에서 메모를 드래그해 위치를 바꿀 수 있습니다.</p>`,`${has?'<button class="btn danger" id="delete-memo">메모 삭제</button>':'<button class="btn" id="cancel-memo">취소</button>'}<button class="btn primary" id="save-memo">저장</button>`);
  const renderPreview=()=>{const box=$('#memo-photo-preview');if(!box)return;const src=memoPhotoSrc(pendingPhoto);box.innerHTML=src?`<div class="memo-photo-preview-card"><div class="memo-photo-preview-image"><img src="${esc(src)}" alt="메모 사진 미리보기"></div><div><b>사진 1장${pendingPhoto?.remoteId?' · 가족 공유':''}</b><small>${pendingPhoto?.size?Math.max(1,Math.round(pendingPhoto.size/1024))+'KB · ':''}앱용으로 자동 압축</small></div><button class="textbtn danger" id="memo-photo-remove">사진 삭제</button></div>`:'<button class="memo-photo-empty" id="memo-photo-empty">＋ 사진을 추가할 수 있습니다</button>';if($('#memo-photo-remove'))$('#memo-photo-remove').onclick=()=>{pendingPhoto=null;photoRemoved=true;renderPreview();};if($('#memo-photo-empty'))$('#memo-photo-empty').onclick=()=>$('#memo-photo-input').click();};
  renderPreview();
  if($('#cancel-memo'))$('#cancel-memo').onclick=closeSheet;
  $('#memo-photo-add').onclick=()=>$('#memo-photo-input').click();
  $('#memo-photo-input').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;const add=$('#memo-photo-add');photoBusy=true;$('#save-memo').disabled=true;add.disabled=true;add.classList.add('loading');toast('사진을 앱용 크기로 준비하고 있습니다.');try{pendingPhoto={...await compressMemoPhoto(file),pendingId:crypto.randomUUID()};photoRemoved=false;renderPreview();toast('사진을 추가했습니다.');}catch(err){toast(err?.message==='size'?'사진 파일이 너무 큽니다. 다른 사진을 선택해 주세요.':'이 사진을 읽지 못했습니다. JPG 또는 PNG 사진을 선택해 주세요.');}finally{photoBusy=false;$('#save-memo').disabled=false;add.disabled=false;add.classList.remove('loading');e.target.value='';}};
  if($('#delete-memo'))$('#delete-memo').onclick=()=>{if(confirm('이 메모를 삭제할까요?'))deleteMemo(id);};
  $('#save-memo').onclick=()=>{
    if(photoBusy)return;
    if(local.memoCards[id]?.deleted){toast('다른 기기에서 삭제된 메모입니다. 새 메모로 추가해 주세요.');return;}
    const text=$('#day-memo').value.trim(),v=$('#memo-position').value;
    if(!text&&!pendingPhoto){toast('메모 내용이나 사진을 추가해 주세요.');return;}
    // Retain uploads completed while the editor was open unless the photo was edited.
    const latest=local.memoCards[id];
    const photo=!photoRemoved&&pendingPhoto===existingPhoto&&latest?latest.photo:pendingPhoto;
    const position=v==='bottom'?null:Math.max(0,Math.min(rows.length,Number(v)||0));
    patchMemo(id,{routeKey:rk,text,position,order:existing?.position===position?(latest?.order||0):Math.max(-1,...routeMemos(rk).filter(m=>m.position===position).map(m=>m.order))+1,photo,deleted:false});
    if(save()){closeSheet();render();toast(photo&&!photo.remoteId?'메모를 저장했습니다. 사진은 연결되면 업로드합니다.':'메모를 저장했습니다.');}
  };
}
function addPlace(){selected=null;const cityName=cityInfo().nameKo;openSheet(`${cityName} 일정에 장소 추가`,`<span class="eyebrow">${dateLabel(date)}</span><label class="field"><span>${cityName} 장소 검색</span><input id="add-search" type="search" placeholder="기존 장소 이름 검색"></label><div id="add-results"></div><p class="notice section">기존 장소 ${db.places.length}곳에서 먼저 찾습니다. 없는 장소는 온라인으로 검색할 수 있습니다.</p><button class="btn" id="external-search">새 장소 온라인 검색</button>`,`<button class="btn primary" id="confirm-add" disabled>장소를 선택해 주세요</button>`);const update=()=>{const q=$('#add-search').value.trim().toLowerCase();const rows=[...db.places,...local.custom].filter(p=>p.city===city&&(p.name+' '+p.en).toLowerCase().includes(q));$('#add-results').innerHTML=rows.map(p=>`<button class="result" data-select="${esc(key(p))}"><b>${esc(p.name)}</b><small>${esc(p.cat)}</small></button>`).join('')||'<p>검색 결과가 없습니다.</p>';document.querySelectorAll('[data-select]').forEach(b=>b.onclick=()=>{selected=b.dataset.select;document.querySelectorAll('[data-select]').forEach(x=>x.classList.toggle('selected',x===b));$('#confirm-add').disabled=false;$('#confirm-add').textContent='이 일정에 추가';});};$('#add-search').oninput=update;$('#external-search').onclick=externalSearch;$('#confirm-add').onclick=()=>{if(!selected)return;if(route().includes(selected)){toast('이미 이 일정에 있는 장소입니다.');return;}local.routes[routeKey()]=[...route(),selected];save();closeSheet();render();toast('일정에 장소를 추가했습니다.');};update();}
function externalSearch(){const cid=city,c=cityInfo();let found=[];selected=null;openSheet('새 장소 찾기',`<label class="field"><span>${c.nameKo} 장소 검색</span><input id="online-query" placeholder="장소명 입력" maxlength="150"></label><button class="btn" id="search-online">검색</button><div id="online-results" aria-live="polite"></div><label class="field section"><span>분류</span><select id="online-cat">${['관광','음식','카페','숙소','쇼핑','마사지','교통'].map(v=>`<option>${v}</option>`).join('')}</select></label><label class="field"><span>간단한 메모</span><textarea id="online-memo" maxlength="1000"></textarea></label><p class="notice">OpenStreetMap 검색 결과입니다. 주소가 방문하려는 지역과 맞는지 확인해 주세요.</p>`,`<button class="btn primary" id="online-save" disabled>장소를 선택해 주세요</button>`);$('#search-online').onclick=async()=>{const q=$('#online-query').value.trim();if(!q)return;const button=$('#search-online');button.disabled=true;$('#online-results').textContent='검색 중…';selected=null;$('#online-save').disabled=true;try{const u=new URL('https://nominatim.openstreetmap.org/search');u.search=new URLSearchParams({format:'jsonv2',q:q+', '+c.name+', Vietnam',countrycodes:'vn',limit:'5','accept-language':'ko,en'});const r=await fetch(u,{signal:AbortSignal.timeout(12000)});if(!r.ok)throw Error();found=await r.json();if(!$('#online-results'))return;$('#online-results').innerHTML=found.map((p,i)=>`<button class="result" data-online="${i}"><b>${esc(p.name||q)}</b><small>${esc(p.display_name)}</small></button>`).join('')||'<p>검색 결과가 없습니다. 다른 이름으로 검색해 주세요.</p>';document.querySelectorAll('[data-online]').forEach(b=>b.onclick=()=>{selected=+b.dataset.online;document.querySelectorAll('[data-online]').forEach(x=>x.classList.toggle('selected',x===b));$('#online-save').disabled=false;$('#online-save').textContent='장소 저장하고 일정에 추가';});}catch{if($('#online-results'))$('#online-results').textContent='온라인 검색에 연결하지 못했습니다. 기존 장소 검색은 계속 이용할 수 있습니다.';}finally{button.disabled=false;}};$('#online-save').onclick=()=>{if(selected===null)return;const x=found[selected];const p={id:'custom-'+crypto.randomUUID(),city:cid,name:x.name||$('#online-query').value.trim(),en:x.name||'',lat:Number(x.lat),lng:Number(x.lon),address:x.display_name,cat:$('#online-cat').value,tip:$('#online-memo').value.trim(),desc:'사용자가 온라인 검색으로 추가한 장소',custom:true};local.custom.push(p);local.routes[routeKey()]=[...route(),key(p)];save();closeSheet();render();toast('새 장소를 저장했습니다.');};}
function mapView(){
  ...
  const draw=all=>{
    ...
    $('#map-order').innerHTML=all?'':`<h2 class="map-order-title">방문 순서</h2><div class="map-order-list">${(()=>{let out='';for(let gap=0;gap<=rows.length;gap++){out+=memosAt(gap,rows.length).map(m=>compactMemo(m,'map-route-memo')).join('');if(gap<rows.length){const p=rows[gap],k=key(p),isDone=todayMapMode&&tdone.has(k),isNext=todayMapMode&&todayNext===k;out+=`<button class="${isDone?'done':''} ${isNext?'next':''}" data-place="${esc(k)}"><span>${isDone?'✓':gap+1}</span><b>${esc(p.name)}${isNext?' · 다음':''}</b></button>`;}}return out;})()}</div>`;
    bindPlaces();bindMemos();map=L.map('map');L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap contributors',maxZoom:19}).addTo(map);const coords=[];
...
function backupStats(data=local){
  return {editedDays:Object.keys(data.routes||{}).length,notes:MemoCards.list(data).length,memoPhotos:MemoCards.list(data).filter(m=>m.photo).length,saved:Array.isArray(data.saved)?data.saved.length:0,custom:Array.isArray(data.custom)?data.custom.length:0,aiUndo:Object.keys(data.aiUndo||{}).length,todayDays:Object.keys(data.todayProgress||{}).length,checklistDays:Object.keys(data.checklists||{}).length};
}
function backupPackage(data=local){return {version:11,schemaVersion:3,appVersion:APP_VERSION,exportedAt:new Date().toISOString(),data};}
...
function validateBackup(v){
  ...
  if(d.notePositions!==undefined){if(!d.notePositions||typeof d.notePositions!=='object'||Array.isArray(d.notePositions)||!Object.entries(d.notePositions).every(([rk,n])=>validRouteKey(rk)&&Number.isInteger(n)&&n>=0&&n<=100))throw Error('notePositions');}
  if(d.memoPhotos!==undefined){if(!d.memoPhotos||typeof d.memoPhotos!=='object'||Array.isArray(d.memoPhotos)||!Object.entries(d.memoPhotos).every(([rk,x])=>{if(!validRouteKey(rk)||!x||typeof x!=='object')return false;const localOk=typeof x.dataUrl==='string'&&x.dataUrl.startsWith('data:image/jpeg;base64,')&&x.dataUrl.length<500000;const remoteUrl=String(x.remoteUrl||x.url||'');const remoteOk=typeof x.remoteId==='string'&&x.remoteId.length>=20&&x.remoteId.length<=100&&/^https:\/\//.test(remoteUrl);return (localOk||remoteOk)&&Number.isFinite(Number(x.width||0))&&Number.isFinite(Number(x.height||0));}))throw Error('memoPhotos');}
  ...
  if(d.memoCards!==undefined&&!MemoCards.valid(d.memoCards,validRouteKey))throw Error('memoCards');
  MemoCards.migrate(d);
...
async function previewBackupFile(file){
  if(!file)return;if(file.size>12e6){toast('백업 파일이 너무 큽니다.');return;}
  try{
    ...
    $('#restore').onclick=()=>{
      ...
      local=checked.data;if(!local.aiUndo||typeof local.aiUndo!=='object'||Array.isArray(local.aiUndo))local.aiUndo={};if(!local.meta||typeof local.meta!=='object'||Array.isArray(local.meta))local.meta={};if(!local.todayProgress||typeof local.todayProgress!=='object'||Array.isArray(local.todayProgress))local.todayProgress={};if(!local.checklists||typeof local.checklists!=='object'||Array.isArray(local.checklists))local.checklists={};if(!local.notePositions||typeof local.notePositions!=='object'||Array.isArray(local.notePositions))local.notePositions={};if(!local.memoPhotos||typeof local.memoPhotos!=='object'||Array.isArray(local.memoPhotos))local.memoPhotos={};
...
function rollbackPreRestore(){
  ...
  $('#cancel-rollback').onclick=closeSheet;$('#do-rollback').onclick=()=>{local=snap.data;if(!local.aiUndo||typeof local.aiUndo!=='object'||Array.isArray(local.aiUndo))local.aiUndo={};if(!local.meta||typeof local.meta!=='object'||Array.isArray(local.meta))local.meta={};if(!local.todayProgress||typeof local.todayProgress!=='object'||Array.isArray(local.todayProgress))local.todayProgress={};if(!local.checklists||typeof local.checklists!=='object'||Array.isArray(local.checklists))local.checklists={};if(!local.notePositions||typeof local.notePositions!=='object'||Array.isArray(local.notePositions))local.notePositions={};if(!local.memoPhotos||typeof local.memoPhotos!=='object'||Array.isArray(local.memoPhotos))local.memoPhotos={};if(save()){localStorage.removeItem(PRE_RESTORE_KEY);closeSheet();render();toast('백업 복원 전 상태로 되돌렸습니다.');}};
}
function toolsView(){
  ...
  <div class="stability-card"><div><span class="eyebrow">앱 상태</span><b>v13.2 · ${online?'온라인':'오프라인'}</b><small>편집 일정 ${stats.editedDays}일 · 저장 장소 ${stats.saved}곳 · 추가 장소 ${stats.custom}곳</small></div><button class="btn" id="check-update">새 버전 확인</button></div>
  ...
  <details class="section"><summary>내 일정 백업·복원</summary><p class="notice">편집한 일정·메모·메모 사진·저장 장소·Today Mode 진행·체크리스트·AI 복원 기록은 이 기기에 저장됩니다. 가족 동기화를 연결하지 않은 기기에서는 이 기기에만 저장됩니다.<br>마지막 백업: ${esc(formatStamp(lastBackup))}</p><div class="actions"><button class="btn" id="export">백업 내려받기</button><button class="btn" id="import">백업 복원</button>${snap?'<button class="btn" id="rollback">복원 전 상태 되돌리기</button>':''}</div><input type="file" id="import-file" accept=".json,application/json" hidden></details>
  <p class="notice">v13 가족 여행지도 · 기존 장소 ${db.places.length}곳 · 로컬 장소 사진 81장 반영 · 전체 사진 슬롯 ${db.places.length}곳 · 외부 대체사진 13곳<br>Today Mode 현장 화면, 백업 복원 전 자동 임시 보관, AI 오류 재시도, 앱 캐시 갱신 확인을 지원합니다.</p>`;
...
function buildAIContext({cid,day,rows,transfer,snapshot,weatherForRequest,adaptive}){
  const common={
    ...
    currentRouteNote:routeMemos(snapshot.routeKey).map(m=>m.text).filter(Boolean).join('\n'),currentTransfer:transfer||null,plannedItineraries:plans(),
...
async function recoverRouteSuggestion(endpoint,answer,snapshot){
  ...
  const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify({message:prompt,history:[],context:{appVersion:'12.4-route-recovery',currentCity:snapshot.city,currentDate:snapshot.date,currentRoute:snapshot.original.map(place).filter(Boolean)}})});
  if(!r.ok)return null;
  const d=await r.json();
...
async function init(){try{const names=['trip','cities','places','itineraries','safety'];const values=await Promise.all(names.map(async n=>{const r=await fetch('data/'+n+'.json?v=13.2');if(!r.ok)throw Error();return r.json();}));db=Object.fromEntries(names.map((n,i)=>[n,values[i]]));const requested=new URLSearchParams(location.search).get('city');if(db.cities.some(c=>c.id===requested))city=requested;date=cityInfo().dates[0];if(syncToActualTripDate())tab='today';render();window.addEventListener('offline',()=>toast('인터넷 연결이 끊겼습니다. 저장된 일정은 계속 볼 수 있습니다.'));window.addEventListener('online',()=>toast('인터넷 연결이 복구되었습니다.'));if('serviceWorker'in navigator){try{swReg=await navigator.serviceWorker.register('./service-worker.js?v=13.2',{updateViaCache:'none'});swReg.update().catch(()=>{});updateLegacyPreviewWorker();}catch{}}}catch{$('#main').innerHTML='<h1>일정을 불러오지 못했습니다</h1><p>인터넷 연결을 확인하고 다시 열어 주세요. 파일을 직접 열었다면 GitHub Pages 또는 웹 서버에서 실행해 주세요.</p><button class="btn" onclick="location.reload()">다시 시도</button>';}}
// Ask only this project's old preview registration to fetch its retirement worker.
// Keep editors open; retirement does not erase local data or force a reload.
async function updateLegacyPreviewWorker(){
  if(!navigator.serviceWorker?.getRegistrations)return;
  try{
    const root=new URL('./',location.href),scope=new URL('v11-preview/',root).href;
    for(const registration of await navigator.serviceWorker.getRegistrations()){
      const script=registration.active?.scriptURL;
      if(registration.scope===scope&&script&&new URL(script).pathname===new URL('service-worker.js',scope).pathname){
        await registration.update();
      }
    }
  }catch{/* Retry on the next online app load. */}
}
init();
