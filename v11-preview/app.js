...
function editMemo(){
  ...
  $('#save-memo').onclick=async()=>{const btn=$('#save-memo'),text=$('#day-memo').value.trim(),v=$('#memo-position').value;btn.disabled=true;let finalPhoto=pendingPhoto;try{if(!text&&!finalPhoto){delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];}else{if(text)local.notes[rk]=text;else delete local.notes[rk];if(finalPhoto)local.memoPhotos[rk]=finalPhoto;else if(photoRemoved||!finalPhoto)delete local.memoPhotos[rk];if(v==='bottom')delete local.notePositions[rk];else local.notePositions[rk]=Math.max(0,Math.min(rows.length,Number(v)||0));}if(existingPhoto?.remoteId&&(photoRemoved||!finalPhoto||finalPhoto.remoteId!==existingPhoto.remoteId))window.familyMemoPhotoDelete?.(existingPhoto);if(save()){closeSheet();render();toast(finalPhoto?.remoteId?'메모와 사진을 저장했습니다.':'메모를 저장했습니다.');}}finally{btn.disabled=false;btn.textContent='저장';}};
}
