...
function parseRouteSuggestion(answer,snapshot){
  ...
  return {content:content||'추천 순서를 준비했습니다.',suggestion:{city:snapshot.city,date:snapshot.date,routeKey:snapshot.routeKey,original:[...snapshot.original],suggested:suggested?[...suggested]:null,parseFailed:!suggested,recovered:false}};
}
async function recoverRouteSuggestion(endpoint,answer,snapshot){
  ...
  const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify({message:prompt,history:[],context:{appVersion:'11.5.3-route-recovery',currentCity:snapshot.city,currentDate:snapshot.date,currentRoute:snapshot.original.map(place).filter(Boolean)}})});
  if(!r.ok)return null;
  const d=await r.json();
