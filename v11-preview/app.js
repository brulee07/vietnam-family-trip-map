...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='12.5-preview';
...
let local={routes:{},notes:{},notePositions:{},saved:[],custom:[],aiUndo:{},todayProgress:{},checklists:{},meta:{}};
...
function routeMemoBlock(text,pos,rows,{bottom=false}={}){return `<div class="route-memo-stop route-sort-item${bottom?' route-memo-bottom':''}" data-sort-kind="memo"><span class="route-memo-dot">✎</span><div class="route-drag-shell"><button class="route-memo-card" data-route-memo-edit><small>경로 메모</small><b>${esc(text)}</b></button>${dragHandle('경로 메모')}</div>${edit?`<div class="edit-row route-memo-edit-row"><button data-note-move="-1" ${pos<=0?'disabled':''}>↑ 위로</button><button data-note-move="1" ${pos>=rows.length?'disabled':''}>↓ 아래로</button><button class="danger" data-note-remove>메모 삭제</button></div>`:''}</div>`;}
function scheduleTimeline(rows){const text=local.notes[routeKey()]||'',pos=notePosition(routeKey(),rows.length);let html='';for(let gap=0;gap<=rows.length;gap++){if(text&&pos===gap)html+=routeMemoBlock(text,gap,rows);if(gap<rows.length){const k=rows[gap],p=place(k);if(p)html+=`<div class="stop route-sort-item" data-sort-kind="place" data-sort-key="${esc(k)}"><span class="dot">${gap+1}</span><div class="route-drag-shell"><button class="card card-media" data-place="${esc(k)}">${cardPhoto(p)}<span class="card-copy"><b>${esc(p.name)}</b><small>${esc(p.cat)}${p.stay?' · '+esc(p.stay):''}</small></span></button>${dragHandle(p.name)}</div>${edit?`<div class="edit-row"><button data-move="${gap}" data-delta="-1" ${gap===0?'disabled':''} aria-label="${esc(p.name)} 위로">↑ 위로</button><button data-move="${gap}" data-delta="1" ${gap===rows.length-1?'disabled':''}>↓ 아래로</button><button class="danger" data-remove="${gap}">일정에서 빼기</button></div>`:''}</div>`;}}if(text&&pos===null&&edit)html+=routeMemoBlock(text,rows.length,rows,{bottom:true});return html;}
...
function todayView(){
  ...
  ${notePosition(routeKey(),rows.length)===null?`<section class="today-section today-note"><div class="today-section-head"><div><span class="eyebrow">가족 메모</span><h2>오늘 메모</h2></div>${memoText?'<button class="textbtn" id="today-memo">편집</button>':''}</div>${memoText?`<div class="memo">${esc(memoText)}</div>`:'<button class="today-note-empty" id="today-memo">+ 메모 작성</button>'}</section>`:''}`;
...
function editMemo(){const rows=route(),has=!!(local.notes[routeKey()]||'');openSheet('하루 메모',`<label class="field"><span>${dateLabel(date)}</span><textarea id="day-memo" maxlength="2000" placeholder="예: 이동: 렌트차량">${esc(local.notes[routeKey()]||'')}</textarea></label><label class="field"><span>메모 표시 위치</span><select id="memo-position">${notePlacementOptions(rows)}</select></label><p class="notice">경로 사이를 직접 선택할 수도 있고, 저장 후 일정의 편집 모드에서 메모의 ⠿ 손잡이를 끌어 원하는 위치에 놓을 수도 있습니다. 장소 카드도 같은 방식으로 순서를 바꿀 수 있습니다.</p>`,`${has?'<button class="btn danger" id="delete-memo">메모 삭제</button>':'<button class="btn" id="cancel-memo">취소</button>'}<button class="btn primary" id="save-memo">저장</button>`);if($('#cancel-memo'))$('#cancel-memo').onclick=closeSheet;if($('#delete-memo'))$('#delete-memo').onclick=()=>{delete local.notes[routeKey()];delete local.notePositions[routeKey()];save();closeSheet();render();toast('메모를 삭제했습니다.');};$('#save-memo').onclick=()=>{const text=$('#day-memo').value.trim(),v=$('#memo-position').value;if(!text){delete local.notes[routeKey()];delete local.notePositions[routeKey()];}else{local.notes[routeKey()]=text;if(v==='bottom')delete local.notePositions[routeKey()];else local.notePositions[routeKey()]=Math.max(0,Math.min(rows.length,Number(v)||0));}save();closeSheet();render();};}
...
function rollbackPreRestore(){
  ...
  $('#cancel-rollback').onclick=closeSheet;$('#do-rollback').onclick=()=>{local=snap.data;if(!local.aiUndo||typeof local.aiUndo!=='object'||Array.isArray(local.aiUndo))local.aiUndo={};if(!local.meta||typeof local.meta!=='object'||Array.isArray(local.meta))local.meta={};if(!local.todayProgress||typeof local.todayProgress!=='object'||Array.isArray(local.todayProgress))local.todayProgress={};if(!local.checklists||typeof local.checklists!=='object'||Array.isArray(local.checklists))local.checklists={};if(!local.notePositions||typeof local.notePositions!=='object'||Array.isArray(local.notePositions))local.notePositions={};if(save()){localStorage.removeItem(PRE_RESTORE_KEY);closeSheet();render();toast('백업 복원 전 상태로 되돌렸습니다.');}};
}
function toolsView(){
  ...
  <div class="stability-card"><div><span class="eyebrow">앱 상태</span><b>v12.4 preview · ${online?'온라인':'오프라인'}</b><small>편집 일정 ${stats.editedDays}일 · 저장 장소 ${stats.saved}곳 · 추가 장소 ${stats.custom}곳</small></div><button class="btn" id="check-update">새 버전 확인</button></div>
  ...
  <p class="notice">v12.4 시범판 · 기존 장소 ${db.places.length}곳 · 로컬 사진 52장 1차 반영 · 전체 사진 슬롯 ${db.places.length}곳 · 외부 대체사진 13곳<br>Today Mode 현장 화면, 백업 복원 전 자동 임시 보관, AI 오류 재시도, 앱 캐시 갱신 확인을 지원합니다.</p>`;
