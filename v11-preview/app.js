...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='12.7-preview';
...
function memoPhoto(rk=routeKey()){const x=local.memoPhotos?.[rk];return x&&typeof x.dataUrl==='string'&&x.dataUrl.startsWith('data:image/')?x:null;}
function hasRouteMemo(rk=routeKey()){return !!((local.notes?.[rk]||'').trim()||memoPhoto(rk));}
function memoPhotoImg(photo,cls='memo-photo-thumb'){return photo?`<img class="${cls}" src="${esc(photo.dataUrl)}" alt="메모 첨부 사진">`:'';}
...
function openMemoPhoto(photo){if(!photo)return;openSheet('메모 사진',`<img class="memo-photo-full" src="${esc(photo.dataUrl)}" alt="메모 첨부 사진 크게 보기"><p class="notice">이 사진은 현재 기기에 저장된 메모 사진입니다.</p>`,`<button class="btn primary" id="memo-photo-done">확인</button>`);$('#memo-photo-done').onclick=closeSheet;}
...
function editMemo(){
  ...
  if($('#delete-memo'))$('#delete-memo').onclick=()=>{delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];save();closeSheet();render();toast('메모를 삭제했습니다.');};
  $('#save-memo').onclick=()=>{const text=$('#day-memo').value.trim(),v=$('#memo-position').value;if(!text&&!pendingPhoto){delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];}else{if(text)local.notes[rk]=text;else delete local.notes[rk];if(pendingPhoto)local.memoPhotos[rk]=pendingPhoto;else if(photoRemoved||!pendingPhoto)delete local.memoPhotos[rk];if(v==='bottom')delete local.notePositions[rk];else local.notePositions[rk]=Math.max(0,Math.min(rows.length,Number(v)||0));}if(save()){closeSheet();render();toast('메모를 저장했습니다.');}};
}
...
function validateBackup(v){
  ...
  if(d.memoPhotos!==undefined){if(!d.memoPhotos||typeof d.memoPhotos!=='object'||Array.isArray(d.memoPhotos)||!Object.entries(d.memoPhotos).every(([rk,x])=>validRouteKey(rk)&&x&&typeof x.dataUrl==='string'&&x.dataUrl.startsWith('data:image/jpeg;base64,')&&x.dataUrl.length<500000&&Number.isFinite(Number(x.width))&&Number.isFinite(Number(x.height))))throw Error('memoPhotos');}
