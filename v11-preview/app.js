...
function parseRouteSuggestion(answer,snapshot){
  let content=String(answer||'').trim();
  const matches=[...content.matchAll(/\[\[ROUTE_ORDER:([^\]]+)\]\]/g)];
  if(!matches.length)return {content,suggestion:null};
  const raw=matches.at(-1)[1];
  content=content.replace(/\n?\[\[ROUTE_ORDER:[^\]]+\]\]\s*/g,'').trim();
  const suggested=raw.split('|').map(v=>v.trim()).filter(Boolean);
  if(!sameRouteMembers(snapshot.original,suggested)||suggested.some(k=>!place(k)))return {content,suggestion:null};
