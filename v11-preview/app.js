...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='12.8-preview';
...
function notePosition(rk=routeKey(),count=route().length){const v=local.notePositions?.[rk];return Number.isInteger(v)?Math.max(0,Math.min(v,count)):null;}
function memoPhoto(rk=routeKey()){const x=local.memoPhotos?.[rk];if(!x||typeof x!=='object')return null;const localOk=typeof x.dataUrl==='string'&&x.dataUrl.startsWith('data:image/');const remoteOk=typeof x.remoteUrl==='string'&&/^https:\/\//.test(x.remoteUrl);return (localOk||remoteOk)?x:null;}
function memoPhotoSrc(photo){return photo?.remoteUrl||photo?.dataUrl||'';}
function hasRouteMemo(rk=routeKey()){return !!((local.notes?.[rk]||'').trim()||memoPhoto(rk));}
...
function openMemoPhoto(photo){if(!photo)return;const src=memoPhotoSrc(photo);openSheet('메모 사진',`<img class="memo-photo-full" src="${esc(src)}" alt="메모 첨부 사진 크게 보기"><p class="notice">${photo.remoteUrl?'가족 공유방에 저장된 사진입니다. 가족 기기에서 함께 볼 수 있습니다.':'이 기기에만 저장된 사진입니다.'}</p>`,`<button class="btn primary" id="memo-photo-done">확인</button>`);$('#memo-photo-done').onclick=closeSheet;}
function notePlacementOptions(rows,selected=notePosition(routeKey(),rows.length)){const opts=[`<option value="bottom" ${selected===null?'selected':''}>일정 아래에 표시</option>`,`<option value="0" ${selected===0?'selected':''}>첫 장소 앞</option>`];for(let i=1;i<=rows.length;i++){const a=place(rows[i-1]),b=i<rows.length?place(rows[i]):null,label=b?`${i}번과 ${i+1}번 사이 · ${a?.name||''} → ${b?.name||''}`:`${i}번 뒤 · ${a?.name||''} 다음`;opts.push(`<option value="${i}" ${selected===i?'selected':''}>${esc(label)}</option>`);}return opts.join('');}
...
function routeMemoBlock(text,pos,rows,{bottom=false}={}){const photo=memoPhoto();return `<div class="route-memo-stop route-sort-item${bottom?' route-memo-bottom':''}" data-sort-kind="memo"><span class="route-memo-dot">✎</span><div class="route-drag-shell"><div class="route-memo-card"><button class="route-memo-main" data-route-memo-edit><span><small>경로 메모${photo?' · 사진':''}</small>${text?`<b>${esc(text)}</b>`:'<b class="muted">사진 메모</b>'}</span></button>${photo?`<button class="memo-photo-thumb-button" data-memo-photo-view aria-label="메모 사진 크게 보기">${memoPhotoImg(photo)}</button>`:''}</div>${dragHandle('경로 메모')}</div>${edit?`<div class="edit-row route-memo-edit-row"><button data-note-move="-1" ${pos<=0?'disabled':''}>↑ 위로</button><button data-note-move="1" ${pos>=rows.length?'disabled':''}>↓ 아래로</button><button class="danger" data-note-remove>메모 삭제</button></div>`:''}</div>`;}
function scheduleTimeline(rows){const text=local.notes[routeKey()]||'',has=hasRouteMemo(),pos=notePosition(routeKey(),rows.length);let html='';for(let gap=0;gap<=rows.length;gap++){if(has&&pos===gap)html+=routeMemoBlock(text,gap,rows);if(gap<rows.length){const k=rows[gap],p=place(k);if(p)html+=`<div class="stop route-sort-item" data-sort-kind="place" data-sort-key="${esc(k)}"><span class="dot">${gap+1}</span><div class="route-drag-shell"><button class="card card-media" data-place="${esc(k)}">${cardPhoto(p)}<span class="card-copy"><b>${esc(p.name)}</b><small>${esc(p.cat)}${p.stay?' · '+esc(p.stay):''}</small></span></button>${dragHandle(p.name)}</div>${edit?`<div class="edit-row"><button data-move="${gap}" data-delta="-1" ${gap===0?'disabled':''} aria-label="${esc(p.name)} 위로">↑ 위로</button><button data-move="${gap}" data-delta="1" ${gap===rows.length-1?'disabled':''}>↓ 아래로</button><button class="danger" data-remove="${gap}">일정에서 빼기</button></div>`:''}</div>`;}}if(has&&pos===null&&edit)html+=routeMemoBlock(text,rows.length,rows,{bottom:true});return html;}
function commitRouteDrag(container,draggedKind,initialNotePos){
  const seq=[...container.querySelectorAll(':scope > [data-sort-kind]')].map(el=>({kind:el.dataset.sortKind,key:el.dataset.sortKey||''}));
  const places=seq.filter(x=>x.kind==='place').map(x=>x.key).filter(Boolean),rk=routeKey();
  local.routes[rk]=places;
  if(hasRouteMemo(rk)){
    const mi=seq.findIndex(x=>x.kind==='memo');
    if(mi>=0){
      const before=seq.slice(0,mi).filter(x=>x.kind==='place').length;
      if(initialNotePos===null&&draggedKind!=='memo'&&mi===seq.length-1)delete local.notePositions[rk];
      else local.notePositions[rk]=Math.max(0,Math.min(places.length,before));
    }
  }
  save();render();toast(draggedKind==='memo'?'메모 위치를 변경했습니다.':'경로 순서를 변경했습니다.');
}
function bindRouteDrag(){
  ...
  container.querySelectorAll('[data-route-drag-handle]').forEach(handle=>handle.addEventListener('pointerdown',e=>{
    ...
    const initialNotePos=notePosition(routeKey(),route().length),draggedKind=item.dataset.sortKind,rect=item.getBoundingClientRect(),offsetY=e.clientY-rect.top;
    ...
    const finish=ev=>{
      ...
      commitRouteDrag(container,draggedKind,initialNotePos);
    };
    const cancel=()=>{
...
function todayView(){
  ...
  const memoText=local.notes[routeKey()]||'',memoPic=memoPhoto();
  ...
  ${notePosition(routeKey(),rows.length)===null?`<section class="today-section today-note"><div class="today-section-head"><div><span class="eyebrow">가족 메모</span><h2>오늘 메모</h2></div>${hasRouteMemo()?'<button class="textbtn" id="today-memo">편집</button>':''}</div>${hasRouteMemo()?`<button class="memo today-memo-card" id="today-memo">${memoPic?memoPhotoImg(memoPic,'today-note-photo'):''}${memoText?`<span>${esc(memoText)}</span>`:'<span>사진 메모</span>'}</button>`:'<button class="today-note-empty" id="today-memo">+ 메모 작성</button>'}</section>`:''}`;
  ...
  $('#today-map').onclick=()=>go('map',true);$('#today-schedule').onclick=()=>go('schedule');if($('#today-memo'))$('#today-memo').onclick=editMemo;if($('[data-today-route-memo]'))$('[data-today-route-memo]').onclick=editMemo;bindPlaces();
...
function schedule(){const rows=route(), ps=plans();$('#main').innerHTML=`<div class="heading"><div><span class="eyebrow">2027. 1. 11 — 1. 25</span><h1>베트남 가족여행</h1><span class="muted">${cityInfo().nameKo} · ${city==='nha_trang'?'가족 9명':'가족 7명'}</span></div><button class="iconbtn" id="open-map" aria-label="일정 지도 보기">◇</button></div>${citySelect()}<div class="date-strip" aria-label="날짜 선택">${cityInfo().dates.map(d=>`<button class="date ${d===date?'active':''}" data-date="${d}" aria-label="${dateLabel(d)}" aria-pressed="${d===date}">${new Date(d+'T12:00Z').toLocaleDateString('ko',{weekday:'short',timeZone:'UTC'})}<b>${Number(d.slice(-2))}</b></button>`).join('')}</div><div class="day-head"><h2>Day ${Number(date.slice(-2))-10} <small>${date.slice(5).replace('-','.')} · ${cityInfo().nameKo}</small></h2><button class="textbtn" id="edit">${edit?'완료':'편집'}</button></div>${edit?'<div class="route-drag-tip"><span>⠿</span> 손잡이를 끌어 장소와 메모 순서를 바꿀 수 있습니다.</div>':''}${aiUndoScheduleBanner()}${ps.length>1?`<label class="field plan-select"><span>일정안 선택</span><select id="plan">${ps.map(p=>`<option value="${p.id}">${esc(p.label)}</option>`).join('')}<option value="custom" ${local.routes[routeKey()]?'selected':''}>내가 편집한 일정</option></select></label>`:''}<div class="timeline" id="schedule-timeline">${scheduleTimeline(rows)}</div>${!rows.length?'<div class="empty">아직 등록된 일정이 없습니다.<br>장소를 추가해 하루를 구성해 보세요.</div>':''}${hasRouteMemo()&&notePosition(routeKey(),rows.length)===null&&!edit?`<button class="memo schedule-bottom-memo" id="bottom-memo">${memoPhoto()?memoPhotoImg(memoPhoto(),'schedule-bottom-photo'):''}${local.notes[routeKey()]?`<span>${esc(local.notes[routeKey()])}</span>`:'<span>사진 메모</span>'}</button>`:''}<div class="actions"><button class="btn" id="add">＋ 장소 추가</button><button class="btn" id="memo">메모 ${hasRouteMemo()?'편집':'추가'}</button></div>${ps[0]?.note?`<details class="section"><summary>기존 일정 메모</summary><p class="notice">${esc(ps[0].note)}</p></details>`:''}<p class="notice">기존에 정리한 일정안입니다. 방문 확정을 뜻하지 않으며, 가족 상황에 맞게 편집할 수 있습니다.</p>`;
$('#open-map').onclick=()=>go('map');$('#edit').onclick=()=>{edit=!edit;render();};$('#add').onclick=addPlace;$('#memo').onclick=editMemo;if($('#bottom-memo'))$('#bottom-memo').onclick=editMemo;if($('#ai-undo-current'))$('#ai-undo-current').onclick=()=>restoreAiRoute(routeKey());document.querySelectorAll('[data-date]').forEach(b=>b.onclick=()=>{date=b.dataset.date;edit=false;render();});bindPlaces();bindRouteDrag();document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{const a=[...route()],i=+b.dataset.remove,pos=notePosition(routeKey(),a.length);a.splice(i,1);local.routes[routeKey()]=a;if(pos!==null&&pos>i)local.notePositions[routeKey()]=Math.max(0,pos-1);save();render();});document.querySelectorAll('[data-move]').forEach(b=>b.onclick=()=>{const a=[...route()],i=+b.dataset.move,j=i+(+b.dataset.delta);[a[i],a[j]]=[a[j],a[i]];local.routes[routeKey()]=a;save();render();});document.querySelectorAll('[data-route-memo-edit]').forEach(b=>b.onclick=editMemo);document.querySelectorAll('[data-memo-photo-view]').forEach(b=>b.onclick=e=>{e.stopPropagation();openMemoPhoto(memoPhoto());});document.querySelectorAll('[data-note-move]').forEach(b=>b.onclick=()=>{let cur=notePosition(routeKey(),rows.length);if(cur===null)cur=rows.length;local.notePositions[routeKey()]=Math.max(0,Math.min(rows.length,cur+(+b.dataset.noteMove)));save();render();});if($('[data-note-remove]'))$('[data-note-remove]').onclick=()=>{if(!confirm('이 메모를 삭제할까요?'))return;const old=memoPhoto(routeKey())?.remoteId;if(old&&window.familyPhotoDelete)window.familyPhotoDelete(old).catch(()=>{});delete local.notes[routeKey()];delete local.notePositions[routeKey()];delete local.memoPhotos[routeKey()];save();render();toast('메모를 삭제했습니다.');};if($('#plan'))$('#plan').onchange=e=>{if(e.target.value==='custom')return;const apply=()=>{local.routes[routeKey()]=db.itineraries.find(p=>p.id===e.target.value).placeIds.map(id=>city+':'+id);save();closeSheet();render();};openSheet('일정안 변경',`<p>선택한 일정안으로 이 날짜의 장소 순서를 교체할까요? 메모와 저장한 장소는 유지됩니다.</p>`,`<button class="btn" id="cancel-plan">취소</button><button class="btn primary" id="apply-plan">교체</button>`);$('#cancel-plan').onclick=()=>{closeSheet();render();};$('#apply-plan').onclick=apply;};}
...
function editMemo(){
  const rows=route(),rk=routeKey(),existingPhoto=memoPhoto(rk);let pendingPhoto=existingPhoto,photoRemoved=false;
  const has=hasRouteMemo(rk),photoIcon=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5h10.5v2H6v9h12v-5h2v7H4z"/><path d="M7 14l2.5-3 2.2 2.4 1.8-2 3.5 4.1H7z"/><path d="M18 2v3h3v2h-3v3h-2V7h-3V5h3V2z"/></svg>`;
  openSheet('하루 메모',`<div class="memo-editor-top"><span>${dateLabel(date)}</span><button class="memo-photo-add" id="memo-photo-add" aria-label="사진 추가">${photoIcon}</button><input type="file" id="memo-photo-input" accept="image/*" hidden></div><label class="field memo-text-field"><textarea id="day-memo" maxlength="2000" placeholder="잊기 쉬운 정보를 메모해보세요.">${esc(local.notes[rk]||'')}</textarea></label><div id="memo-photo-preview" class="memo-photo-preview"></div><label class="field"><span>메모 표시 위치</span><select id="memo-position">${notePlacementOptions(rows)}</select></label><p class="notice">가족 공유방에 연결된 상태에서는 사진도 가족 기기에 함께 동기화됩니다. 오프라인이거나 공유방을 사용하지 않으면 이 기기에만 저장됩니다.</p>`,`${has?'<button class="btn danger" id="delete-memo">메모 삭제</button>':'<button class="btn" id="cancel-memo">취소</button>'}<button class="btn primary" id="save-memo">저장</button>`);
  const renderPreview=()=>{const box=$('#memo-photo-preview');if(!box)return;const src=memoPhotoSrc(pendingPhoto);box.innerHTML=src?`<div class="memo-photo-preview-card"><div class="memo-photo-preview-image"><img src="${esc(src)}" alt="메모 사진 미리보기"></div><div><b>사진 1장${pendingPhoto?.remoteUrl?' · 가족 공유':''}</b><small>${Math.max(1,Math.round((pendingPhoto?.size||0)/1024))}KB · ${pendingPhoto?.remoteUrl?'R2 저장':'앱용 자동 압축'}</small></div><button class="textbtn danger" id="memo-photo-remove">사진 삭제</button></div>`:'<button class="memo-photo-empty" id="memo-photo-empty">＋ 사진을 추가할 수 있습니다</button>';if($('#memo-photo-remove'))$('#memo-photo-remove').onclick=()=>{pendingPhoto=null;photoRemoved=true;renderPreview();};if($('#memo-photo-empty'))$('#memo-photo-empty').onclick=()=>$('#memo-photo-input').click();};
  renderPreview();
  if($('#cancel-memo'))$('#cancel-memo').onclick=closeSheet;
  $('#memo-photo-add').onclick=()=>$('#memo-photo-input').click();
  $('#memo-photo-input').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;const add=$('#memo-photo-add');add.disabled=true;add.classList.add('loading');toast('사진을 앱용 크기로 준비하고 있습니다.');try{pendingPhoto=await compressMemoPhoto(file);photoRemoved=false;renderPreview();toast('사진을 추가했습니다.');}catch(err){toast(err?.message==='size'?'사진 파일이 너무 큽니다. 다른 사진을 선택해 주세요.':'이 사진을 읽지 못했습니다. JPG 또는 PNG 사진을 선택해 주세요.');}finally{add.disabled=false;add.classList.remove('loading');e.target.value='';}};
  if($('#delete-memo'))$('#delete-memo').onclick=async()=>{const old=existingPhoto?.remoteId;if(!confirm('이 메모를 삭제할까요?'))return;if(old&&window.familyPhotoDelete)window.familyPhotoDelete(old).catch(()=>{});delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];save();closeSheet();render();toast('메모를 삭제했습니다.');};
  $('#save-memo').onclick=async()=>{
    const btn=$('#save-memo'),text=$('#day-memo').value.trim(),v=$('#memo-position').value;btn.disabled=true;btn.textContent='저장 중…';
    try{
      let finalPhoto=pendingPhoto;
      if(finalPhoto?.dataUrl&&!finalPhoto?.remoteUrl&&window.familyPhotoSyncReady?.()){
        toast('사진을 가족 공유방에 업로드하고 있습니다.');
        try{const remote=await window.familyPhotoUpload(finalPhoto);if(remote)finalPhoto={...remote,name:finalPhoto.name,width:finalPhoto.width,height:finalPhoto.height,size:finalPhoto.size,updatedAt:new Date().toISOString()};}
        catch{toast('사진 업로드에 실패해 이 기기에만 저장합니다. 나중에 다시 편집해 저장하면 재시도할 수 있습니다.');}
      }
      const oldRemote=existingPhoto?.remoteId,newRemote=finalPhoto?.remoteId;
      if(!text&&!finalPhoto){delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];}
      else{if(text)local.notes[rk]=text;else delete local.notes[rk];if(finalPhoto)local.memoPhotos[rk]=finalPhoto;else delete local.memoPhotos[rk];if(v==='bottom')delete local.notePositions[rk];else local.notePositions[rk]=Math.max(0,Math.min(rows.length,Number(v)||0));}
      if(save()){
        if(oldRemote&&oldRemote!==newRemote&&window.familyPhotoDelete)window.familyPhotoDelete(oldRemote).catch(()=>{});
        closeSheet();render();toast(finalPhoto?.remoteUrl?'메모와 사진을 가족과 공유했습니다.':'메모를 저장했습니다.');
      }
    }finally{if($('#save-memo')){$('#save-memo').disabled=false;$('#save-memo').textContent='저장';}}
  };
}
...
function mapView(){
  ...
  const draw=all=>{
    ...
    $('#map-order').innerHTML=all?'':`<h2 class="map-order-title">방문 순서</h2><div class="map-order-list">${(()=>{const memo=local.notes[routeKey()]||'',pos=notePosition(routeKey(),rows.length);let out='';for(let gap=0;gap<=rows.length;gap++){if(hasRouteMemo()&&pos===gap)out+=`<button class="map-route-memo" data-map-route-memo><span>✎</span><b>${memo?esc(memo):'사진 메모'}${memoPhoto()?' · 📷':''}</b></button>`;if(gap<rows.length){const p=rows[gap],k=key(p),isDone=todayMapMode&&tdone.has(k),isNext=todayMapMode&&todayNext===k;out+=`<button class="${isDone?'done':''} ${isNext?'next':''}" data-place="${esc(k)}"><span>${isDone?'✓':gap+1}</span><b>${esc(p.name)}${isNext?' · 다음':''}</b></button>`;}}return out;})()}</div>`;
    bindPlaces();if($('[data-map-route-memo]'))$('[data-map-route-memo]').onclick=editMemo;map=L.map('map');L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap contributors',maxZoom:19}).addTo(map);const coords=[];
...
function backupStats(data=local){
  return {editedDays:Object.keys(data.routes||{}).length,notes:new Set([...Object.keys(data.notes||{}),...Object.keys(data.memoPhotos||{})]).size,memoPhotos:Object.keys(data.memoPhotos||{}).length,saved:Array.isArray(data.saved)?data.saved.length:0,custom:Array.isArray(data.custom)?data.custom.length:0,aiUndo:Object.keys(data.aiUndo||{}).length,todayDays:Object.keys(data.todayProgress||{}).length,checklistDays:Object.keys(data.checklists||{}).length};
}
...
function toolsView(){
  ...
  <div class="stability-card"><div><span class="eyebrow">앱 상태</span><b>v12.7 preview · ${online?'온라인':'오프라인'}</b><small>편집 일정 ${stats.editedDays}일 · 저장 장소 ${stats.saved}곳 · 추가 장소 ${stats.custom}곳</small></div><button class="btn" id="check-update">새 버전 확인</button></div>
  ...
  <p class="notice">v12.7 시범판 · 기존 장소 ${db.places.length}곳 · 로컬 장소 사진 68장 반영 · 전체 사진 슬롯 ${db.places.length}곳 · 외부 대체사진 13곳<br>Today Mode 현장 화면, 백업 복원 전 자동 임시 보관, AI 오류 재시도, 앱 캐시 갱신 확인을 지원합니다.</p>`;
