...
function aiUndoScheduleBanner(){
  ...
  if(sameOrder(current,u.before))return `<div class="ai-undo-banner restored"><div><b>AI 적용 전 순서로 복원됨</b><small>다시 AI 동선 최적화를 실행하면 새 추천을 받을 수 있습니다.</small></div></div>`;
  return '';
}
...
async function recoverRouteSuggestion(endpoint,answer,snapshot){
  ...
  const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify({message:prompt,history:[],context:{appVersion:'11.6-route-recovery',currentCity:snapshot.city,currentDate:snapshot.date,currentRoute:snapshot.original.map(place).filter(Boolean)}})});
  if(!r.ok)return null;
  const d=await r.json();
