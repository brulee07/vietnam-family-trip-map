...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='11.8-preview';
let db, city='da_nang', date='2027-01-11', tab='home', edit=false, map=null, filter='전체', selected=null, chat=[], busy=false, swReg=null;
let local={routes:{},notes:{},saved:[],custom:[],aiUndo:{},meta:{}};
...
function backupStats(data=local){
  return {editedDays:Object.keys(data.routes||{}).length,notes:Object.keys(data.notes||{}).length,saved:Array.isArray(data.saved)?data.saved.length:0,custom:Array.isArray(data.custom)?data.custom.length:0,aiUndo:Object.keys(data.aiUndo||{}).length};
}
...
async function previewBackupFile(file){
  ...
  try{
    ...
    openSheet('백업 복원 확인',`<p>아래 백업으로 이 기기의 편집 내용을 교체할까요?</p><div class="restore-preview"><span><small>편집 일정</small><b>${s.editedDays}일</b></span><span><small>메모</small><b>${s.notes}개</b></span><span><small>저장 장소</small><b>${s.saved}곳</b></span><span><small>추가 장소</small><b>${s.custom}곳</b></span></div><p class="notice">백업 생성: ${esc(formatStamp(checked.exportedAt))}<br>백업 앱: ${esc(checked.appVersion)}<br>현재 내용은 복원 직전 상태로 자동 보관되어 한 번 되돌릴 수 있습니다.</p>`,`<button class="btn" id="cancel-restore">취소</button><button class="btn primary" id="restore">교체하고 복원</button>`);
    ...
    $('#restore').onclick=()=>{
      ...
      local=checked.data;if(!local.aiUndo||typeof local.aiUndo!=='object'||Array.isArray(local.aiUndo))local.aiUndo={};if(!local.meta||typeof local.meta!=='object'||Array.isArray(local.meta))local.meta={};
...
function rollbackPreRestore(){
  ...
  $('#cancel-rollback').onclick=closeSheet;$('#do-rollback').onclick=()=>{local=snap.data;if(!local.aiUndo||typeof local.aiUndo!=='object'||Array.isArray(local.aiUndo))local.aiUndo={};if(!local.meta||typeof local.meta!=='object'||Array.isArray(local.meta))local.meta={};if(save()){localStorage.removeItem(PRE_RESTORE_KEY);closeSheet();render();toast('백업 복원 전 상태로 되돌렸습니다.');}};
}
function toolsView(){
  ...
  <div class="stability-card"><div><span class="eyebrow">앱 상태</span><b>v11.8 preview · ${online?'온라인':'오프라인'}</b><small>편집 일정 ${stats.editedDays}일 · 저장 장소 ${stats.saved}곳 · 추가 장소 ${stats.custom}곳</small></div><button class="btn" id="check-update">새 버전 확인</button></div>
...
async function recoverRouteSuggestion(endpoint,answer,snapshot){
  ...
  const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify({message:prompt,history:[],context:{appVersion:'11.8-route-recovery',currentCity:snapshot.city,currentDate:snapshot.date,currentRoute:snapshot.original.map(place).filter(Boolean)}})});
  if(!r.ok)return null;
  const d=await r.json();
