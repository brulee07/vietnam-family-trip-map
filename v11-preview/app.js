...
function memoPhoto(rk=routeKey()){const x=local.memoPhotos?.[rk];if(!x||typeof x!=='object')return null;const localOk=typeof x.dataUrl==='string'&&x.dataUrl.startsWith('data:image/');const remoteOk=typeof (x.remoteUrl||x.url)==='string'&&/^https:\/\//.test(x.remoteUrl||x.url);return localOk||remoteOk?x:null;}
...
function editMemo(){
  ...
  if($('#delete-memo'))$('#delete-memo').onclick=()=>{const oldPhoto=memoPhoto(rk);if(oldPhoto?.remoteId)window.familyMemoPhotoDelete?.(oldPhoto);delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];save();closeSheet();render();toast('메모를 삭제했습니다.');};
  $('#save-memo').onclick=async()=>{const btn=$('#save-memo'),text=$('#day-memo').value.trim(),v=$('#memo-position').value;btn.disabled=true;let finalPhoto=pendingPhoto;try{if(finalPhoto?.dataUrl&&!finalPhoto?.remoteId&&window.familyMemoPhotoUpload){btn.textContent='사진 동기화 중…';try{finalPhoto=await window.familyMemoPhotoUpload(rk,finalPhoto)||finalPhoto;}catch{toast('사진은 이 기기에 저장했습니다. 온라인 연결 후 가족 동기화를 다시 시도합니다.');}}if(!text&&!finalPhoto){delete local.notes[rk];delete local.notePositions[rk];delete local.memoPhotos[rk];}else{if(text)local.notes[rk]=text;else delete local.notes[rk];if(finalPhoto)local.memoPhotos[rk]=finalPhoto;else if(photoRemoved||!finalPhoto)delete local.memoPhotos[rk];if(v==='bottom')delete local.notePositions[rk];else local.notePositions[rk]=Math.max(0,Math.min(rows.length,Number(v)||0));}if(existingPhoto?.remoteId&&(photoRemoved||!finalPhoto||finalPhoto.remoteId!==existingPhoto.remoteId))window.familyMemoPhotoDelete?.(existingPhoto);if(save()){closeSheet();render();toast(finalPhoto?.remoteId?'메모와 사진을 저장했습니다.':'메모를 저장했습니다.');}}finally{btn.disabled=false;btn.textContent='저장';}};
}
