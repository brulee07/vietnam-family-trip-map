...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='12.3-preview';
...
function toolsView(){
  ...
  <div class="stability-card"><div><span class="eyebrow">앱 상태</span><b>v12.3 preview · ${online?'온라인':'오프라인'}</b><small>편집 일정 ${stats.editedDays}일 · 저장 장소 ${stats.saved}곳 · 추가 장소 ${stats.custom}곳</small></div><button class="btn" id="check-update">새 버전 확인</button></div>
  ...
  <p class="notice">v12.3 시범판 · 기존 장소 ${db.places.length}곳 · 로컬 사진 52장 1차 반영 · 전체 사진 슬롯 ${db.places.length}곳 · 외부 대체사진 13곳<br>Today Mode 현장 화면, 백업 복원 전 자동 임시 보관, AI 오류 재시도, 앱 캐시 갱신 확인을 지원합니다.</p>`;
...
async function recoverRouteSuggestion(endpoint,answer,snapshot){
  ...
  const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify({message:prompt,history:[],context:{appVersion:'12.3-route-recovery',currentCity:snapshot.city,currentDate:snapshot.date,currentRoute:snapshot.original.map(place).filter(Boolean)}})});
  if(!r.ok)return null;
  const d=await r.json();
