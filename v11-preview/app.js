...
const KEY='familyTravelV11', PRE_RESTORE_KEY='familyTravelV11PreRestore', APP_VERSION='11.9-preview';
let db, city='da_nang', date='2027-01-11', tab='home', edit=false, map=null, filter='전체', selected=null, chat=[], busy=false, swReg=null;
...
function go(t){if(t==='today'&&db)syncToActualTripDate();tab=t;edit=false;render();window.scrollTo(0,0);}
...
function setTodayDone(k,done){const rk=routeKey(),p=todayProgress(rk),next=new Set(p.done);if(done)next.add(k);else next.delete(k);local.todayProgress[rk]={done:[...next],updatedAt:new Date().toISOString()};save();render();}
...
function todayView(){
  ...
  const nextRow=rows.find(x=>!done.has(x.k)),nextIndex=nextRow?rows.findIndex(x=>x.k===nextRow.k):-1,allDone=rows.length>0&&doneCount===rows.length;
  const w=liveWeather?.cityId===city?liveWeather.current:null,transfer=db.trip.route.find(r=>r.date===date),cs=citiesForDate(date),checklist=checklistFor(),checkDone=checklist.filter(x=>x.done).length;
  ...
  const nextBlock=allDone?`<section class="today-next complete"><span class="eyebrow">오늘 일정 완료</span><h2>모든 장소를 확인했어요</h2><p>필요하면 진행 표시를 초기화해 다시 사용할 수 있습니다.</p><button class="btn" id="today-reset">진행 표시 초기화</button></section>`:nextRow?`<section class="today-next"><span class="eyebrow">${nextIndex===0?'시작 장소':'다음 장소'}</span><h2>${esc(nextRow.p.name)}</h2><p>${esc(nextRow.p.cat)}${nextRow.p.stay?' · '+esc(nextRow.p.stay):''}</p>${nextRow.p.tip?`<small>${esc(nextRow.p.tip)}</small>`:''}<div class="today-next-actions"><a class="btn primary" href="${esc(mapsURL(nextRow.p))}" target="_blank" rel="noopener">지금 출발</a><button class="btn" id="today-arrive">${nextIndex===0?'시작 완료':'도착·완료'}</button></div></section>`:`<section class="today-next"><span class="eyebrow">오늘 일정</span><h2>등록된 장소가 없습니다</h2><p>일정 화면에서 장소를 추가해 주세요.</p><button class="btn primary" id="today-edit-schedule">일정 만들기</button></section>`;
  ...
  if($('#today-arrive'))$('#today-arrive').onclick=()=>setTodayDone(nextRow.k,true);if($('#today-reset'))$('#today-reset').onclick=resetTodayProgress;if($('#today-edit-schedule'))$('#today-edit-schedule').onclick=()=>go('schedule');
  $('#today-map').onclick=()=>go('map');$('#today-schedule').onclick=()=>go('schedule');$('#today-memo').onclick=editMemo;bindPlaces();
...
function toolsView(){
  ...
  <div class="stability-card"><div><span class="eyebrow">앱 상태</span><b>v11.9 preview · ${online?'온라인':'오프라인'}</b><small>편집 일정 ${stats.editedDays}일 · 저장 장소 ${stats.saved}곳 · 추가 장소 ${stats.custom}곳</small></div><button class="btn" id="check-update">새 버전 확인</button></div>
  ...
  <p class="notice">v11.9 시범판 · 기존 장소 ${db.places.length}곳 · 로컬 사진 슬롯 ${db.places.length}곳 · 외부 대체사진 13곳<br>Today Mode 현장 화면, 백업 복원 전 자동 임시 보관, AI 오류 재시도, 앱 캐시 갱신 확인을 지원합니다.</p>`;
...
async function recoverRouteSuggestion(endpoint,answer,snapshot){
  ...
  const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify({message:prompt,history:[],context:{appVersion:'11.9-route-recovery',currentCity:snapshot.city,currentDate:snapshot.date,currentRoute:snapshot.original.map(place).filter(Boolean)}})});
  if(!r.ok)return null;
  const d=await r.json();
