...
function loadConfig(){
  ...
  return {enabled:!!v.enabled,endpoint:String(v.endpoint||endpointDefault()),roomCode:String(v.roomCode||''),token:String(v.token||''),revision:Number(v.revision||0),deviceId:String(v.deviceId||makeId()),deviceName:String(v.deviceName||'가족 휴대폰').slice(0,40),lastSyncAt:v.lastSyncAt||null,lastError:v.lastError||null,dirty:!!v.dirty};
}
let cfg=loadConfig();
...
function syncErrorText(e){if(e?.status===401||e?.status===403)return '공유방 인증정보가 맞지 않습니다.';if(e?.status===404)return '공유방을 찾지 못했습니다.';if(e?.status===409)return '가족의 최신 변경사항과 합치는 중 충돌이 발생했습니다.';if(e?.name==='TimeoutError')return '동기화 서버 응답이 늦습니다.';if(navigator.onLine===false)return '현재 오프라인입니다.';return '가족 동기화 서버에 연결하지 못했습니다.';}
async function pullRemote(){
  const d=await api('/sync/pull',{...roomPayload(),revision:cfg.revision});if(d.notModified)return null;validateRemoteState(d.state);cfg.revision=Number(d.revision||0);saveCfg();return d;
}
async function pushState(state,baseRevision=cfg.revision){
  try{return await api('/sync/push',{...roomPayload(),baseRevision:Number(baseRevision||0),state});}
  catch(e){if(e.status===409&&e.data?.state){validateRemoteState(e.data.state);const merged=mergeShared(state,e.data.state);applyShared(merged,{renderNow:false});const retried=await api('/sync/push',{...roomPayload(),baseRevision:Number(e.data.revision||0),state:merged});return retried;}throw e;}
}
async function syncNow(mode='manual'){
  ...
  try{
    ...
    cfg.lastSyncAt=nowIso();cfg.lastError=null;saveCfg();if(remoteChanged||tab==='tools'||tab==='today')render();return true;
...
function openSyncServerSheet(){openSheet('가족 동기화 서버',`<p>가족 공유용 Cloudflare Sync Worker 주소를 입력하세요.</p><label class="field"><span>Sync Worker 주소</span><input id="sync-endpoint" inputmode="url" value="${esc(cfg.endpoint||endpointDefault())}" placeholder="https://...workers.dev"></label><label class="field"><span>이 기기 이름</span><input id="sync-device-name" maxlength="40" value="${esc(cfg.deviceName)}"></label><p class="notice">AI Worker와 별도의 주소입니다. 가족 일정 데이터만 저장합니다.</p>`,`<button class="btn" id="sync-server-cancel">취소</button><button class="btn primary" id="sync-server-save">저장</button>`);$('#sync-server-cancel').onclick=closeSheet;$('#sync-server-save').onclick=()=>{try{cfg.endpoint=normalizeEndpoint($('#sync-endpoint').value);cfg.deviceName=$('#sync-device-name').value.trim().slice(0,40)||'가족 휴대폰';saveCfg();closeSheet();render();toast('동기화 서버 설정을 저장했습니다.');}catch{toast('https:// Worker 주소를 확인해 주세요.');}};}
async function createRoom(){
  ...
  syncBusy=true;renderSyncBits();try{
    ...
    cfg.enabled=true;cfg.roomCode=String(d.code||'');cfg.token=String(d.token||'');cfg.revision=Number(d.revision||1);cfg.dirty=false;cfg.lastSyncAt=nowIso();cfg.lastError=null;saveCfg();render();openInviteSheet();
...
async function previewJoin(code,token,deviceName){
  ...
  try{const d=await api('/sync/pull',{...roomPayload(),code,token},15000);validateRemoteState(d.state);cfg.deviceName=String(deviceName||cfg.deviceName).trim().slice(0,40)||'가족 휴대폰';const s=previewStats(d.state),localStats=previewStats(sharedState());openSheet('가족 일정 연결 확인',`<p><b>${esc(code)}</b> 공유방의 일정입니다.</p>${statsHtml(s)}<p class="notice">현재 이 기기: 편집 일정 ${localStats.days}일 · 메모 ${localStats.notes}개 · 저장 장소 ${localStats.saved}곳<br><b>가족 일정으로 맞추기</b>는 이 기기의 공유 대상 데이터를 교체합니다. <b>병합</b>은 양쪽의 최신 변경을 항목별로 합칩니다.</p>`,`<button class="btn" id="sync-join-back">뒤로</button><button class="btn" id="sync-join-merge">병합</button><button class="btn primary" id="sync-join-replace">가족 일정으로 맞추기</button>`);$('#sync-join-back').onclick=()=>openJoinSheet({code,token});$('#sync-join-replace').onclick=()=>finishJoin(d,'replace');$('#sync-join-merge').onclick=()=>finishJoin(d,'merge');}
  catch(e){cfg.roomCode=old.roomCode;cfg.token=old.token;cfg.revision=old.revision;toast(syncErrorText(e));}
}
function finishJoin(d,mode){const state=mode==='merge'?mergeShared(sharedState(),d.state):d.state;applyShared(state);cfg.enabled=true;cfg.revision=Number(d.revision||0);cfg.dirty=mode==='merge';cfg.lastSyncAt=nowIso();cfg.lastError=null;saveCfg();closeSheet();history.replaceState(null,'',location.pathname+location.search);render();toast('가족 공유방에 연결했습니다.');if(cfg.dirty)schedulePush();}
function disconnect(skipConfirm=false){if(!skipConfirm&&!confirm('이 기기만 가족 동기화에서 연결 해제할까요? 기기에 저장된 일정은 그대로 남습니다.'))return;cfg={...cfg,enabled:false,roomCode:'',token:'',revision:0,dirty:false,lastError:null,lastSyncAt:null};saveCfg();clearInterval(syncPollTimer);render();toast('이 기기의 가족 동기화를 해제했습니다.');}
async function deleteRoom(){if(!hasCreds())return;if(!confirm('가족 공유방을 서버에서 삭제할까요? 다른 가족 기기도 더 이상 동기화할 수 없습니다.'))return;try{await api('/sync/delete',roomPayload());disconnect(true);}catch(e){toast(syncErrorText(e));}}
...
function renderSyncPanel(){
  ...
  const connected=hasCreds(),status=syncBusy?['busy','동기화 중']:cfg.lastError?['error','확인 필요']:connected?['connected','연결됨']:['','꺼짐'];
  const html=`<section class="family-sync-card"><div class="family-sync-head"><div><span class="eyebrow">가족 공유</span><b>가족 일정 동기화</b><small>${connected?'일정·메모·체크리스트 변경을 가족 휴대폰과 함께 사용합니다.':'공유방을 만들거나 가족 초대 링크로 참여할 수 있습니다.'}</small></div><span class="sync-status ${status[0]}">${status[1]}</span></div>${connected?`<div class="family-sync-room"><span><small>공유방</small><b class="sync-code">${esc(cfg.roomCode)}</b></span><span><small>마지막 동기화</small><b>${esc(fmt(cfg.lastSyncAt))}</b></span></div>${cfg.lastError?`<p class="family-sync-note">${esc(cfg.lastError)}</p>`:''}<div class="family-sync-actions"><button class="btn primary" id="sync-now">지금 동기화</button><button class="btn" id="sync-copy">초대 링크 복사</button><button class="btn" id="sync-disconnect">이 기기 연결 해제</button></div>`:`<div class="family-sync-actions"><button class="btn primary" id="sync-create">가족 공유 시작</button><button class="btn" id="sync-join">가족 공유 참여</button></div>`}<details class="sync-advanced"><summary>동기화 설정</summary><p class="family-sync-note">서버: ${esc(cfg.endpoint||'설정 안 됨')}<br>기기 이름: ${esc(cfg.deviceName)}</p><div class="family-sync-actions"><button class="btn" id="sync-server">서버·기기 설정</button>${connected?'<button class="btn danger" id="sync-delete-room">공유방 삭제</button>':''}</div></details><p class="family-sync-note">공유 대상: 일정 순서 · 하루 메모 · 저장/추가 장소 · Today 진행 · 체크리스트<br>AI 대화와 AI 복원 기록, 백업 기록, 기기별 설정은 공유하지 않습니다.</p></section>`;
  anchor.insertAdjacentHTML('afterend',html);
  if($('#sync-create'))$('#sync-create').onclick=createRoom;if($('#sync-join'))$('#sync-join').onclick=()=>openJoinSheet();if($('#sync-now'))$('#sync-now').onclick=()=>syncNow('manual');if($('#sync-copy'))$('#sync-copy').onclick=copyInvite;if($('#sync-disconnect'))$('#sync-disconnect').onclick=disconnect;if($('#sync-server'))$('#sync-server').onclick=openSyncServerSheet;if($('#sync-delete-room'))$('#sync-delete-room').onclick=deleteRoom;
}
function renderSyncBits(){if(tab==='tools')renderSyncPanel();const s=document.querySelector('.family-sync-mini span');if(s)s.textContent=syncBusy?'가족 일정 동기화 중…':cfg.lastError?'가족 동기화 확인 필요':hasCreds()?`가족 동기화 · ${fmt(cfg.lastSyncAt)}`:'가족 동기화 꺼짐';}
toolsView=function(){baseToolsView();renderSyncPanel();};
todayView=function(){baseTodayView();if(!hasCreds())return;const heading=document.querySelector('.today-picker');if(heading&&!document.querySelector('.family-sync-mini'))heading.insertAdjacentHTML('afterend',`<div class="family-sync-mini"><span>${syncBusy?'가족 일정 동기화 중…':cfg.lastError?'가족 동기화 확인 필요':`가족 동기화 · ${esc(fmt(cfg.lastSyncAt))}`}</span><button id="sync-mini-open">설정</button></div>`);if($('#sync-mini-open'))$('#sync-mini-open').onclick=()=>go('tools');};
