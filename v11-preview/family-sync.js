...
function loadConfig(){
  ...
  return {enabled:!!v.enabled,endpoint:String(v.endpoint||endpointDefault()),roomCode:String(v.roomCode||''),token:String(v.token||''),revision:Number(v.revision||0),deviceId:String(v.deviceId||makeId()),deviceName:String(v.deviceName||'가족 휴대폰').slice(0,40),lastSyncAt:v.lastSyncAt||null,lastError:v.lastError||null,dirty:!!v.dirty,deviceCount:Number(v.deviceCount||0),maxDevices:Number(v.maxDevices||MAX_FAMILY_DEVICES)};
}
let cfg=loadConfig();
...
function devicesHtml(){if(!hasCreds())return '';if(!syncDevices.length)return `<div class="sync-devices"><div class="sync-devices-title"><b>참여 기기</b><span>${cfg.deviceCount||1}/${cfg.maxDevices||MAX_FAMILY_DEVICES}</span></div><p class="family-sync-note">기기 목록을 불러오는 중입니다.</p></div>`;const rows=[...syncDevices].sort((a,b)=>(a.device_id===cfg.deviceId?-1:b.device_id===cfg.deviceId?1:Date.parse(b.last_seen_at||0)-Date.parse(a.last_seen_at||0))).map(d=>{const st=deviceStatus(d.last_seen_at),me=d.device_id===cfg.deviceId;return `<div class="sync-device-row"><span class="device-dot ${st[0]}"></span><div><b>${esc(d.device_name||'가족 휴대폰')}${me?' <em>이 기기</em>':''}</b><small>${st[1]} · ${esc(relativeTime(d.last_seen_at))}${d.last_sync_at?' · 동기화 '+esc(relativeTime(d.last_sync_at)):''}</small></div>${!me&&st[0]==='offline'?`<button class="device-remove" data-device="${esc(d.device_id)}" data-name="${esc(d.device_name||'가족 휴대폰')}">제거</button>`:''}</div>`;}).join('');return `<div class="sync-devices"><div class="sync-devices-title"><b>참여 기기</b><span>${cfg.deviceCount||syncDevices.length}/${cfg.maxDevices||MAX_FAMILY_DEVICES}</span></div>${rows}<button class="sync-device-refresh" id="sync-device-refresh">기기 목록 새로고침</button></div>`;}
